...
private:
	...
	{
		if (value < current->data) //넣을 값이 현재 노드의 값보다 작으면
		{
			if (!current->left) //왼쪽으로 가는데 비어있다면
				current->left = new node{ value, NULL, NULL }; //왼쪽노드에 삽입
			else
				insert_impl(current->left, value); //비어있지 않다면 다시 왼쪽으로 타고 내려감 
		}
		else //현재 노드보다 값이 크다면
		{
			if (!current->right) //오른쪽이 비어있으면 
				current->right = new node{ value, NULL, NULL };//바로 넣고
			else
				insert_impl(current->right, value); //비어있을때까지 들어간다
		}
	}
